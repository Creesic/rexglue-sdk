...
void FM2ProducerProgressGuardEntry82369340(uint64_t& lr) {
  if (!REXCVAR_GET(fm2_prod_guard_stats)) {
    return;
  }
  ProdGuardHitCaller(static_cast<uint32_t>(lr));
}

void FM2ProducerProgressGuardFlagBlocked82369390(uint64_t& lr, PPCCRRegister& cr0) {
  if (cr0.eq) {
    return;
  }
  if (!REXCVAR_GET(fm2_prod_guard_stats)) {
...
void FM2ProducerProgressGuardWaitDetail823693F8(uint64_t& lr, PPCRegister& delta,
    ...
    if (sample_interval != 0u) {
      ...
      if ((prior % sample_interval) == 0u) {
        ...
        LogLineProdGuard(
            ...
            static_cast<unsigned int>(static_cast<uint32_t>(lr)), dlt, static_cast<unsigned int>(r30.u32),
...
void FM2ProducerProgressGuardTimeoutDetail82369400(uint64_t& lr, PPCRegister& delta) {
