...
VSOutput VSMain(VSInput input) {
  const float3 stream0_position =
      float3(asfloat(input.position0.x), asfloat(input.position0.y),
             asfloat(input.position0.z));
  ...
  const uint packed_color =
      input.side0.x ^ (input.side0.y >> 3u) ^ input.side0.z ^ input.vertex_id;
  ...
  if (constants.transform_valid != 0u && constants.transform_mode == 1u) {
    ...
    output.position =
        float4(clip_position.x, clip_position.y, clip_position.w * 0.5f,
               clip_position.w);
  }
  output.color = float4(
      saturate(stream0_position.z + 0.5f),
      float((packed_color >> 8u) & 0xFFu) / 255.0f,
      float((packed_color >> 16u) & 0xFFu) / 255.0f, 1.0f);
  return output;
}
