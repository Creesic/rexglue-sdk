  ...
  for (uint32_t i = 0; i < edram_bases_sorted_count; ++i) {
    ...
    rt_lengths_tiles[i] = std::min(std::min(length_used_tiles_at_32bpp << rt_is_64bpp,
                                   ...
                                   ((i + 1 < edram_bases_sorted_count)
                                       ...
                                       rt_base);
    // A lone render target's distance-to-next-base clamp above degenerates to the
    // whole EDRAM, letting it wrap around and claim tiles owned by another render
    // target at a lower base; clamp it here so it can't wrap past the end.
