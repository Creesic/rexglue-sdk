...
void SpirvShaderTranslator::StartTranslation() {
  ...
  if (!is_depth_only_fragment_shader_) {
    ...
    if (IsSpirvRectangleListVertexLoopEnabled()) {
      ...
      var_main_rectangle_positions_ = builder_->createVariable(
          spv::NoPrecision, spv::StorageClassFunction, type_rectangle_position_array,
          "xe_var_rectangle_position");
      uint32_t rectangle_clip_distance_count = GetSpirvShaderModification().GetVertexClipDistanceCount();
      if (rectangle_clip_distance_count) {
        spv::Id type_rectangle_clip_distance =
            builder_->makeArrayType(type_float_, builder_->makeUintConstant(rectangle_clip_distance_count), 0);
      ...
      uint32_t rectangle_cull_distance_count = GetSpirvShaderModification().GetVertexCullDistanceCount();
      if (rectangle_cull_distance_count) {
        spv::Id type_rectangle_cull_distance =
            builder_->makeArrayType(type_float_, builder_->makeUintConstant(rectangle_cull_distance_count), 0);
  ...
  if (!is_depth_only_fragment_shader_) {
    ...
    if (rectangle_vertex_loop) {
      ...
      builder_->createStore(
          builder_->createLoad(builder_->createAccessChain(spv::StorageClassOutput, output_per_vertex_,
      ...
      for (uint32_t i = 0; i < clip_distance_count; ++i) {
        ...
        builder_->createStore(
            builder_->createLoad(builder_->createAccessChain(spv::StorageClassOutput, output_per_vertex_,
                                                             rectangle_member_indices),
                                 spv::NoPrecision),
            builder_->createAccessChain(spv::StorageClassFunction, var_main_rectangle_clip_distances_,
      ...
      for (uint32_t i = 0; i < cull_distance_count; ++i) {
        ...
        builder_->createStore(
            builder_->createLoad(builder_->createAccessChain(spv::StorageClassOutput, output_per_vertex_,
                                                             rectangle_member_indices),
                                 spv::NoPrecision),
            builder_->createAccessChain(spv::StorageClassFunction, var_main_rectangle_cull_distances_,
      ...
      if_not_last_guest_vertex.makeBeginElse();
      {
        ...
        auto load_rectangle_distance_component = [this](spv::Id vertex_index, spv::Id component_index,
      ...
      builder_->createConditionalBranch(rectangle_guest_vertex_is_last, main_loop_exit_,
                                        main_loop_continue_);
    }
