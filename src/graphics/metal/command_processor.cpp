...
#include <rex/graphics/metal/upload_buffer_pool.h>
...
uint64_t MetalCommandProcessor::GetLatestSubmissionStarted() const {
...
bool MetalCommandProcessor::SetupContext() {
  std::fprintf(stderr, "[probe] MetalCommandProcessor::SetupContext enter\n");
  std::fflush(stderr);
  saw_swap_ = false;
  last_swap_ptr_ = 0;
  last_swap_width_ = 0;
  ...
  if (!CommandProcessor::SetupContext()) {
    std::fprintf(stderr, "[probe] MetalCommandProcessor::SetupContext base context failed\n");
    std::fflush(stderr);
    return false;
  }
  ...
  if (!device_ || !mtl4_) {
    ...
    std::fprintf(stderr, "[probe] MetalCommandProcessor::SetupContext missing device or mtl4\n");
    std::fflush(stderr);
    return false;
  }
  std::fprintf(stderr, "[probe] MetalCommandProcessor::SetupContext device=%p mtl4=%p\n",
               static_cast<void*>(device_), static_cast<void*>(mtl4_));
  std::fflush(stderr);
  ...
  if (!shared_memory_->Initialize()) {
    std::fprintf(stderr, "[probe] MetalCommandProcessor::SetupContext shared_memory init failed\n");
    std::fflush(stderr);
    return false;
  }
  std::fprintf(stderr, "[probe] MetalCommandProcessor::SetupContext shared_memory ok\n");
  std::fflush(stderr);
  ...
  if (!primitive_processor_->Initialize()) {
    std::fprintf(stderr,
                 "[probe] MetalCommandProcessor::SetupContext primitive_processor init failed\n");
    std::fflush(stderr);
    return false;
  }
  std::fprintf(stderr, "[probe] MetalCommandProcessor::SetupContext primitive_processor ok\n");
  std::fflush(stderr);
  ...
  if (!view_bindless_heap_) {
    ...
    std::fprintf(stderr, "[probe] MetalCommandProcessor::SetupContext view_bindless_heap failed\n");
    std::fflush(stderr);
    return false;
  }
  ...
  if (!sampler_bindless_heap_) {
    ...
    std::fprintf(stderr,
                 "[probe] MetalCommandProcessor::SetupContext sampler_bindless_heap failed\n");
    std::fflush(stderr);
    return false;
  }
  ...
  if (!texture_cache_->Initialize()) {
    std::fprintf(stderr, "[probe] MetalCommandProcessor::SetupContext texture_cache init failed\n");
    std::fflush(stderr);
    return false;
  }
  std::fprintf(stderr, "[probe] MetalCommandProcessor::SetupContext texture_cache ok\n");
  std::fflush(stderr);
  ...
  if (!render_target_cache_->Initialize()) {
    std::fprintf(stderr,
                 "[probe] MetalCommandProcessor::SetupContext render_target_cache init failed\n");
    std::fflush(stderr);
    return false;
  }
  std::fprintf(stderr, "[probe] MetalCommandProcessor::SetupContext render_target_cache ok\n");
  std::fflush(stderr);
      ...
      std::make_unique<MetalPipelineCache>(device_, *register_file_);
  {
    ...
    if (!pipeline_cache_->InitializeShaderTranslation(
      ...
      std::fprintf(stderr,
                   "[probe] MetalCommandProcessor::SetupContext pipeline shader translation failed\n");
      std::fflush(stderr);
      return false;
    }
  }
  std::fprintf(stderr, "[probe] MetalCommandProcessor::SetupContext pipeline shader translation ok\n");
  std::fflush(stderr);
  if (mesh_shader_supported_) {
    ...
    if (!tessellator_tables_buffer_) {
      REXLOG_ERROR("Failed to allocate tessellator tables buffer ({} bytes)",
             tess_tables_size);
      std::fprintf(stderr,
                   "[probe] MetalCommandProcessor::SetupContext tessellator tables alloc failed\n");
      std::fflush(stderr);
      return false;
    }
  ...
  if (!null_buffer_) {
    ...
    std::fprintf(stderr, "[probe] MetalCommandProcessor::SetupContext null_buffer failed\n");
    std::fflush(stderr);
    return false;
  }
  ...
  if (!null_texture_) {
    ...
    std::fprintf(stderr, "[probe] MetalCommandProcessor::SetupContext null_texture failed\n");
    std::fflush(stderr);
    return false;
  }
  ...
  if (!null_sampler_) {
    ...
    std::fprintf(stderr, "[probe] MetalCommandProcessor::SetupContext null_sampler failed\n");
    std::fflush(stderr);
    return false;
  }
  ...
  if (!system_view_tables_) {
    ...
    std::fprintf(stderr, "[probe] MetalCommandProcessor::SetupContext system_view_tables failed\n");
    std::fflush(stderr);
    return false;
  }
              ...
              system_view_tables_->length());
  {
    ...
    if (!render_target_cache_ ||
      ...
      std::fprintf(stderr,
                   "[probe] MetalCommandProcessor::SetupContext system edram descriptor failed A\n");
      std::fflush(stderr);
      return false;
    }
    ...
    if (!render_target_cache_ ||
      ...
      REXLOG_ERROR("Failed to encode typed EDRAM UAV system descriptor");
      std::fprintf(stderr,
                   "[probe] MetalCommandProcessor::SetupContext system edram descriptor failed B\n");
      std::fflush(stderr);
      return false;
    }
  }

  mtl4_->AddResidentAllocation(view_bindless_heap_);
  mtl4_->AddResidentAllocation(sampler_bindless_heap_);
  mtl4_->AddResidentAllocation(null_buffer_);
  mtl4_->AddResidentAllocation(system_view_tables_);
  if (null_texture_) mtl4_->AddResidentAllocation(null_texture_);
  if (tessellator_tables_buffer_) {
    mtl4_->AddResidentAllocation(tessellator_tables_buffer_);
  }
  if (shared_memory_) {
    MTL::Buffer* shared_mem_buffer = shared_memory_->GetBuffer();
    if (shared_mem_buffer) mtl4_->AddResidentAllocation(shared_mem_buffer);
  }
  mtl4_->CommitResidency();
  std::fprintf(stderr, "[probe] MetalCommandProcessor::SetupContext success\n");
  std::fflush(stderr);
...
void MetalCommandProcessor::IssueSwap(uint32_t frontbuffer_ptr,
  ...
  if (presenter && render_target_cache_) {
    ...
    if (REXCVAR_QUERY(bool, metal_present_probe)) {
      ...
      if (swap_probe_count < 16) {
        ++swap_probe_count;
        REXLOG_WARN(
            "[swap-src-probe] src_tex={} src={}x{} out={}x{} ptr={:08X} rb_swap={} pwl_gamma={}",
            fmt::ptr(source_texture), uint32_t(source_texture->width()),
            uint32_t(source_texture->height()), output_width, output_height,
            frontbuffer_ptr, force_swap_rb, use_pwl_gamma_ramp);
      }
    }
...
bool MetalCommandProcessor::IssueDraw(xenos::PrimitiveType primitive_type,
  ...
  if (shared_memory_) {
    ...
    for (const auto& binding : vb_bindings) {
      ...
      rex_assert(vertex_range_count < vertex_ranges.size());
...
bool MetalCommandProcessor::UploadConstants(
  ...
  draw_util::GetHostViewportInfo(
      regs, draw_resolution_scale_x, draw_resolution_scale_y,
  ...
  if (any_cbuffer_dirty) {
    ...
    if (!vs_data || !ps_data) {
      REXLOG_ERROR("IssueDraw: constant buffer pool allocation failed");
      return false;
    }
...
bool MetalCommandProcessor::PopulateBindlessTables(
  ...
  if (!reuse_bindless_table) {
    ...
    auto track_texture_usage = [&](MTL::Texture* texture) {
      ...
      rex_assert(textures_for_encoder_count < textures_for_encoder.size());
...
bool MetalCommandProcessor::DispatchDraw(
  ...
  if (use_tessellation_emulation) {
    ...
    mtl4_tess_config.tessellatorOutputPrimitive =
        (IRRuntimeTessellatorOutputPrimitive)tess_config.outputPrimitiveType;
...
void MetalCommandProcessor::BeginCommandBuffer() {
  ...
  if (auto* ca = mtl4_pass_descriptor->colorAttachments()->object(0)) {
    if (auto* tex = ca->texture()) {
      mtl4_->AddResidentAllocation(tex);
    }
  }
  if (auto* da = mtl4_pass_descriptor->depthAttachment()) {
    if (auto* tex = da->texture()) {
      mtl4_->AddResidentAllocation(tex);
    }
  }
...
void MetalCommandProcessor::UseRenderEncoderResource(MTL::Resource* resource,
  ...
  // MTL4::RenderCommandEncoder doesn't have useResource; resource usage
  // is tracked through argument tables in the Metal4 path.
  (void)resource;
  (void)usage;
}

bool MetalCommandProcessor::HasActiveSubmission() const {
  return current_mtl4_command_buffer_ != nullptr;
}

bool MetalCommandProcessor::CanJoinActiveSubmissionForTransfer() const {
  return HasActiveSubmission() && !submission_has_draws_;
}
