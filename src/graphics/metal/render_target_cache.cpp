...
#include <rex/graphics/shaders/bytecode/metal/host_depth_store_1xmsaa_cs.h>
#include <rex/graphics/shaders/bytecode/metal/host_depth_store_2xmsaa_cs.h>
#include <rex/graphics/shaders/bytecode/metal/host_depth_store_4xmsaa_cs.h>
#include <rex/graphics/shaders/bytecode/metal/resolve_fast_32bpp_1x2xmsaa_cs.h>
#include <rex/graphics/shaders/bytecode/metal/resolve_fast_32bpp_1x2xmsaa_scaled_cs.h>
#include <rex/graphics/shaders/bytecode/metal/resolve_fast_32bpp_4xmsaa_cs.h>
#include <rex/graphics/shaders/bytecode/metal/resolve_fast_32bpp_4xmsaa_scaled_cs.h>
#include <rex/graphics/shaders/bytecode/metal/resolve_fast_64bpp_1x2xmsaa_cs.h>
#include <rex/graphics/shaders/bytecode/metal/resolve_fast_64bpp_1x2xmsaa_scaled_cs.h>
#include <rex/graphics/shaders/bytecode/metal/resolve_fast_64bpp_4xmsaa_cs.h>
#include <rex/graphics/shaders/bytecode/metal/resolve_fast_64bpp_4xmsaa_scaled_cs.h>
#include <rex/graphics/shaders/bytecode/metal/resolve_full_128bpp_cs.h>
#include <rex/graphics/shaders/bytecode/metal/resolve_full_128bpp_scaled_cs.h>
#include <rex/graphics/shaders/bytecode/metal/resolve_full_16bpp_cs.h>
#include <rex/graphics/shaders/bytecode/metal/resolve_full_16bpp_scaled_cs.h>
#include <rex/graphics/shaders/bytecode/metal/resolve_full_32bpp_cs.h>
#include <rex/graphics/shaders/bytecode/metal/resolve_full_32bpp_scaled_cs.h>
#include <rex/graphics/shaders/bytecode/metal/resolve_full_64bpp_cs.h>
#include <rex/graphics/shaders/bytecode/metal/resolve_full_64bpp_scaled_cs.h>
#include <rex/graphics/shaders/bytecode/metal/resolve_full_8bpp_cs.h>
#include <rex/graphics/shaders/bytecode/metal/resolve_full_8bpp_scaled_cs.h>
...
const MetalRenderTargetCache::TransferModeInfo MetalRenderTargetCache::kTransferModeInfos[] = {
    {MetalRenderTargetCache::TransferOutput::kColor, true, false},        // kColorToColor
    {MetalRenderTargetCache::TransferOutput::kDepth, true, false},        // kColorToDepth
    {MetalRenderTargetCache::TransferOutput::kColor, false, false},       // kDepthToColor
    {MetalRenderTargetCache::TransferOutput::kDepth, false, false},       // kDepthToDepth
    {MetalRenderTargetCache::TransferOutput::kStencilBit, true, false},   // kColorToStencilBit
    {MetalRenderTargetCache::TransferOutput::kStencilBit, false, false},  // kDepthToStencilBit
    {MetalRenderTargetCache::TransferOutput::kDepth, true, true},         // kColorAndHostDepthToDepth
    {MetalRenderTargetCache::TransferOutput::kDepth, false, true},        // kDepthAndHostDepthToDepth
};
...
bool MetalRenderTargetCache::InitializeEdramBufferViews() {
  ...
  for (const ViewInit& view_init : kViews) {
    ...
    if (!width) {
      REXLOG_ERROR("MetalRenderTargetCache: invalid EDRAM bindless width for {}",
             view_init.label);
      ReleaseEdramBufferViews();
      return false;
    }
    ...
    desc->release();
    if (!texture) {
      REXLOG_ERROR("MetalRenderTargetCache: failed to create EDRAM bindless view {}",
             view_init.label);
      ReleaseEdramBufferViews();
      return false;
    }
...
bool MetalRenderTargetCache::Initialize() {
  ...
  if (!device_) {
    REXLOG_ERROR("MetalRenderTargetCache: No Metal device available");
    return false;
  }
  ...
  gamma_render_target_as_unorm16_ = REXCVAR_GET(gamma_render_target_as_unorm16) &&
                                    REXCVAR_GET(metal_allow_gamma_unorm16);
  if (REXCVAR_GET(gamma_render_target_as_unorm16) &&
      !REXCVAR_GET(metal_allow_gamma_unorm16)) {
    REXLOG_WARN(
  ...
  if (!InitializeEdramBufferViews()) {
    REXLOG_ERROR("MetalRenderTargetCache: Failed to create EDRAM bindless views");
    return false;
  }
...
void MetalRenderTargetCache::Shutdown(bool from_destructor) {
  ...
  for (size_t i = 0; i < rex::countof(transfer_dummy_color_float_); ++i) {
...
bool MetalRenderTargetCache::InitializeEdramComputeShaders() {
  ...
  if (!resolve_full_8bpp_pipeline_ || !resolve_full_16bpp_pipeline_ ||
      ...
      !resolve_fast_64bpp_1x2xmsaa_pipeline_ ||
      !resolve_fast_64bpp_4xmsaa_pipeline_) {
    REXLOG_ERROR("Metal: failed to initialize resolve compute pipelines");
    return false;
  }
  ...
  if (draw_resolution_scaled) {
    ...
    if (!resolve_full_8bpp_scaled_pipeline_ ||
        ...
        !resolve_fast_64bpp_1x2xmsaa_scaled_pipeline_ ||
        !resolve_fast_64bpp_4xmsaa_scaled_pipeline_) {
      REXLOG_ERROR("Metal: failed to initialize scaled resolve compute pipelines");
      return false;
    }
  }
  ...
  for (size_t i = 0; i < rex::countof(host_depth_store_pipelines_); ++i) {
    if (!host_depth_store_pipelines_[i]) {
      REXLOG_ERROR("Metal: failed to initialize host depth store pipelines");
      return false;
    }
  }
...
kernel void DUMP_KERNEL_NAME(
  ...
  for (const auto& cfg : kEdramDumpConfigs) {
    ...
    if (!lib) {
      REXLOG_WARN("Metal: failed to compile {} shader: {}", cfg.kernel_name,
             error ? error->localizedDescription()->utf8String() : "unknown");
      continue;
    }
    NS::String* fn_name =
    ...
    MTL::Function* fn = lib->newFunction(fn_name);
    if (!fn) {
      REXLOG_WARN("Metal: {} missing entrypoint", cfg.kernel_name);
      lib->release();
      continue;
    }
    *cfg.pipeline = device_->newComputePipelineState(fn, &error);
    ...
    lib->release();
    if (!*cfg.pipeline) {
      REXLOG_WARN("Metal: failed to create {} pipeline: {}", cfg.kernel_name,
             error ? error->localizedDescription()->utf8String() : "unknown");
    }
  }
...
void MetalRenderTargetCache::BeginFrame() {
  ...
  if (REXCVAR_GET(metal_memory_log_rate) > 0 &&
      (frame_id_ % uint64_t(REXCVAR_GET(metal_memory_log_rate))) == 0) {
    REXLOG_INFO(
...
bool MetalRenderTargetCache::Update(
  ...
  if (!RenderTargetCache::Update(is_rasterization_done,
    ...
    REXLOG_ERROR("MetalRenderTargetCache::Update - Base class Update failed");
    return false;
  }

  if (REXCVAR_GET(metal_memory_log_rate) > 0) {
    static uint64_t memory_log_counter = 0;
    if ((++memory_log_counter % uint64_t(REXCVAR_GET(metal_memory_log_rate))) ==
        0) {
      REXLOG_INFO(
          "Metal mem: frame={} rt={} map={} dummy={} pipelines={} "
          "tile_pipelines={} inst_buf_sizes=[{}, {}, {}]",
...
RenderTargetCache::RenderTarget* MetalRenderTargetCache::CreateRenderTarget(
  ...
  if (!key.is_depth) {
    ...
    if (draw_format != resource_format) {
      ...
      if (!draw_view) {
        REXLOG_ERROR("Failed to create texture view for render target");
      }
...
MTL::PixelFormat MetalRenderTargetCache::GetColorResourcePixelFormat(
  ...
  switch (format) {
    ...
    default:
      REXLOG_ERROR("MetalRenderTargetCache: Unsupported color format {}",
...
MTL::PixelFormat MetalRenderTargetCache::GetDepthPixelFormat(
  ...
  switch (format) {
    ...
    default:
      REXLOG_ERROR("MetalRenderTargetCache: Unsupported depth format {}",
             static_cast<uint32_t>(format));
      return MTL::PixelFormatDepth32Float_Stencil8;
  }
}
...
void MetalRenderTargetCache::StoreTiledData(
    MTL4::CommandBuffer* command_buffer, MTL::Texture* texture,
    uint32_t edram_base, uint32_t pitch_tiles, uint32_t height_tiles,
    bool is_depth) {
  ...
  if (texture->textureType() == MTL::TextureType2DMultisample) {
    ...
    if (resolve_desc) {
      ...
      color_attachment->setResolveTexture(temp_texture);
      color_attachment->setLoadAction(MTL::LoadActionLoad);
  ...
  MTL4::ComputeCommandEncoder* encoder = command_buffer->computeCommandEncoder();
  if (!encoder) {
    if (temp_texture) {
  ...
  if (!edram_store_pipeline_) {
    REXLOG_ERROR("StoreTiledData: edram_store_pipeline_ is null");
    encoder->endEncoding();
    if (temp_texture) {
      temp_texture->release();
    }
  ...
  Metal4Context* mtl4 = command_processor_.GetMetal4Context();
  mtl4->SetComputeTexture(source_texture->gpuResourceID(), 0);
  mtl4->SetComputeAddress(edram_buffer_->gpuAddress(), 0);

  mtl4->SetComputeAddress(mtl4->AllocInlineConstant(&edram_base, sizeof(uint32_t)), 1);
  mtl4->SetComputeAddress(mtl4->AllocInlineConstant(&pitch_tiles, sizeof(uint32_t)), 2);
...
void MetalRenderTargetCache::DumpRenderTargets(
    ...
    uint32_t dump_pitch, MTL4::CommandBuffer* command_buffer) {
  REXGPU_DEBUG(
  ...
  REXGPU_DEBUG("MetalRenderTargetCache::DumpRenderTargets: {} rectangles to dump",
           rectangles.size());
  if (rectangles.empty()) {
    REXLOG_WARN(
  ...
  if (!cmd) {
    ...
    if (!cmd) {
      REXLOG_ERROR("MetalRenderTargetCache::DumpRenderTargets: no command buffer");
      return;
    }
    standalone = true;
  }
  ...
  for (const ResolveCopyDumpRectangle& rect : rectangles) {
    ...
    if (key.is_depth) {
      ...
      assert_true2(tex->pixelFormat() == expected_format, "Dump depth must bind resource pixel format");
    } else {
      MTL::PixelFormat expected_format =
          GetColorResourcePixelFormat(key.GetColorFormat());
      assert_true2(tex->pixelFormat() == expected_format, "Dump color must bind resource pixel format");
    }
    ...
    for (uint32_t i = 0; i < dispatch_count; ++i) {
      ...
      mtl4->SetComputeTexture(tex->gpuResourceID(), 0);
      if (stencil_tex) {
        mtl4->SetComputeTexture(stencil_tex->gpuResourceID(), 1);
      }
      mtl4->SetComputeAddress(mtl4->AllocInlineConstant(&constants, sizeof(constants)), 1);
      mtl4->FlushComputeBindings(encoder);
...
fragment EdramLoadOut edram_load_ps(
  ...
  if (!library) {
    REXLOG_ERROR("Metal: failed to compile edram load shader: {}",
...
MTL::RenderPipelineState* MetalRenderTargetCache::GetOrCreateEdramLoadPipeline(
  ...
  if (!vs || !ps) {
    ...
    REXLOG_ERROR("Metal: edram load missing shader entrypoints");
    return nullptr;
  }
  ...
  if (!pipeline) {
    REXLOG_ERROR("Metal: failed to create edram load pipeline: {}",
           error ? error->localizedDescription()->utf8String() : "unknown");
    return nullptr;
  }
    ...
    if (resolve_info.copy_dest_extent_length) {
      ...
      if (draw_resolution_scaled && !pipeline) {
        ...
        if (missing_scaled_pipeline_log_count < 8) {
          ...
          REXLOG_WARN("MetalResolve: scaled resolve pipeline missing for shader {}",
                 int(copy_shader));
        }
      }
      ...
      if (pipeline && group_count_x && group_count_y) {
        ...
        if (draw_resolution_scaled) {
          ...
          if (!metal_texture_cache->EnsureScaledResolveMemoryCommitted(
            ...
            REXLOG_ERROR("MetalResolve: failed to select scaled resolve buffer");
            return false;
          }
          ...
          if (!dest_buffer) {
            REXLOG_ERROR("MetalResolve: missing shared memory buffer");
            return false;
          }
            ...
            if (!encoder) {
              ...
              if (draw_resolution_scaled) {
                mtl4->SetComputeAddress(
                    mtl4->AllocInlineConstant(&copy_constants.dest_relative,
                                              sizeof(copy_constants.dest_relative)),
                    0);
              } else {
                mtl4->SetComputeAddress(
...
void MetalRenderTargetCache::PerformTransfersAndResolveClears(
  ...
  uint32_t depth_round = (!REXCVAR_GET(depth_float24_convert_in_pixel_shader) &&
                          REXCVAR_GET(depth_float24_round))
                             ? 1u
                             : 0u;
  ...
  if (host_depth_store_needed) {
    MTL4::ComputeCommandEncoder* depth_store_encoder = nullptr;
    for (uint32_t i = 0; i < render_target_count; ++i) {
      RenderTarget* dest_rt = render_targets[i];
      if (!dest_rt) {
        continue;
      ...
      for (const Transfer& transfer : depth_transfers) {
        ...
        if (pipeline_index >= rex::countof(host_depth_store_pipelines_) ||
        ...
        if (!depth_store_encoder) {
          ...
          depth_store_encoder->setComputePipelineState(
              host_depth_store_pipelines_[pipeline_index]);
          Metal4Context* mtl4 = command_processor_.GetMetal4Context();
          mtl4->SetComputeAddress(edram_buffer_->gpuAddress(), 1);
          mtl4->SetComputeTexture(depth_texture->gpuResourceID(), 0);
          mtl4->FlushComputeBindings(depth_store_encoder);
        }
        for (uint32_t rect_index = 0; rect_index < rectangle_count;
          ...
          Metal4Context* mtl4 = command_processor_.GetMetal4Context();
          mtl4->SetComputeAddress(
              mtl4->AllocInlineConstant(&constants, sizeof(constants)), 0);
          mtl4->FlushComputeBindings(depth_store_encoder);
  ...
  for (uint32_t i = 0; i < render_target_count; ++i) {
    ...
    if (dest_is_depth) {
      assert_true2(dest_texture->pixelFormat() == dest_pixel_format, "Transfer depth must use resource pixel format");
    } else {
      assert_true2(dest_texture->pixelFormat() == dest_pixel_format, "Transfer color must use ownership pixel format");
    }

    uint32_t dest_sample_count = MsaaSamplesToCount(dest_key.msaa_samples);
    bool transfer_use_sample_id_default =
        dest_sample_count > 1 && REXCVAR_GET(metal_transfer_msaa_sample_id);
    ...
    auto set_rect_viewport = [&](MTL4::RenderCommandEncoder* encoder,
    ...
    auto allocate_instance_buffer = [&](size_t size, MTL::Buffer*& buffer,
      ...
      size_t aligned_offset =
          rex::align(static_cast<size_t>(transfer_tile_instance_buffer_offset_), kAlignment);
      ...
      if (!transfer_tile_instance_buffers_[buffer_index] ||
          transfer_tile_instance_buffer_sizes_[buffer_index] < required) {
        size_t new_size = rex::round_up<size_t>(required, 65536);
    ...
    auto build_tile_batches =
      ...
      for (uint32_t rect_index = 0; rect_index < rectangle_count;
        ...
        total_instance_bytes = rex::align(total_instance_bytes, kAlignment);
    ...
    MTL4::ComputeCommandEncoder* blit_encoder = nullptr;
    auto ensure_blit_encoder = [&]() -> MTL4::ComputeCommandEncoder* {
      if (!blit_encoder) {
        blit_encoder = cmd->computeCommandEncoder();
      }
      return blit_encoder;
    };
    ...
    if (!transfers.empty()) {
      auto try_blit_transfer = [&](const Transfer& transfer) -> bool {
        ...
        if (!dest_is_depth) {
          MTL::PixelFormat expected_format =
              GetColorOwnershipTransferPixelFormat(source_key.GetColorFormat(),
                                                   nullptr);
          assert_true2(source_texture->pixelFormat() == expected_format, "Transfer source must use ownership pixel format");
        }
        ...
        MTL4::ComputeCommandEncoder* blit = ensure_blit_encoder();
        if (!blit) {
          return false;
        }
    ...
    MTL4::RenderCommandEncoder* transfer_encoder = nullptr;
    auto ensure_transfer_encoder = [&]() -> MTL4::RenderCommandEncoder* {
      if (transfer_encoder) {
    ...
    if (!transfers_for_shaders.empty() && disable_transfer_shaders) {
      ...
      if (stencil_clear_needed) {
        ...
        if (clear_pipeline && stencil_clear_state) {
          MTL4::RenderCommandEncoder* encoder = ensure_transfer_encoder();
          if (encoder) {
            ...
            Metal4Context* mtl4 = command_processor_.GetMetal4Context();
            mtl4->SetFragmentAddress(
                mtl4->AllocInlineConstant(&constants, sizeof(constants)), 0);
            mtl4->FlushRenderBindings(encoder);
            for (const Transfer& transfer : transfers_for_shaders) {
              Transfer::Rectangle
      ...
      if (encoder) {
        ...
        auto bind_transfer_fragment_texture = [&](uint32_t index,
          ...
          if (last_transfer_fragment_textures[index] != texture) {
            mtl4->SetFragmentTexture(texture->gpuResourceID(), index);
            mtl4->FlushRenderBindings(encoder);
        ...
        auto bind_transfer_fragment_buffer_1 = [&](MTL::Buffer* buffer) {
          if (last_transfer_fragment_buffer_1 != buffer) {
            mtl4->SetFragmentAddress(buffer->gpuAddress(), 1);
            mtl4->FlushRenderBindings(encoder);
            ...
            [&](const TransferShaderConstants& constants) {
              if (!transfer_constants_valid ||
                  std::memcmp(&last_transfer_constants, &constants,
                              sizeof(constants)) != 0) {
                MTL::GPUAddress addr =
                    mtl4->AllocInlineConstant(&constants, sizeof(constants));
                mtl4->SetVertexAddress(addr, 0);
                mtl4->SetFragmentAddress(addr, 0);
                mtl4->FlushRenderBindings(encoder);
        ...
        auto bind_transfer_vertex_buffer_1 = [&](MTL::Buffer* buffer,
          ...
          if (last_transfer_vertex_slot_1_binding !=
            ...
            mtl4->SetVertexAddress(buffer->gpuAddress() + offset, 1);
            mtl4->FlushRenderBindings(encoder);
        ...
        auto bind_transfer_vertex_bytes_1 =
            [&](const TransferRectInstance& rect_instance) {
              if (last_transfer_vertex_slot_1_binding !=
                ...
                mtl4->SetVertexAddress(
                    mtl4->AllocInlineConstant(&rect_instance,
        ...
        for (size_t invocation_index = 0;
          ...
          if (mode_info.source_is_color) {
            ...
            assert_true2(source_texture->pixelFormat() == source_transfer_format, "Transfer source must use ownership pixel format");
          ...
          if (REXCVAR_GET(metal_transfer_tile_instancing)) {
    ...
    if (resolve_clear_needed && !resolve_clear_via_load_action) {
      ...
      if (dest_is_depth) {
        ...
        if (clear_pipeline && clear_state) {
          MTL4::RenderCommandEncoder* clear_encoder = ensure_transfer_encoder();
          if (clear_encoder) {
            TransferClearDepthConstants constants = {};
            ...
            clear_encoder->setStencilReferenceValue(uint32_t(clear_value) &
                                                    0xFF);
            Metal4Context* mtl4_clear = command_processor_.GetMetal4Context();
            mtl4_clear->SetFragmentAddress(
                mtl4_clear->AllocInlineConstant(&constants, sizeof(constants)), 0);
            mtl4_clear->FlushRenderBindings(clear_encoder);
        ...
        if (clear_texture) {
          ...
          if (clear_pipeline) {
            MTL4::RenderCommandEncoder* clear_encoder =
                ensure_transfer_encoder();
            if (clear_encoder) {
              MTL::DepthStencilState* no_depth_state =
                  GetTransferNoDepthStencilState();
              ...
              Metal4Context* mtl4_color = command_processor_.GetMetal4Context();
              if (clear_use_uint) {
                mtl4_color->SetFragmentAddress(
                    mtl4_color->AllocInlineConstant(&uint_constants,
                                              sizeof(uint_constants)),
                    0);
              } else {
                mtl4_color->SetFragmentAddress(
                    mtl4_color->AllocInlineConstant(&float_constants,
                                              sizeof(float_constants)),
                    0);
              }
              mtl4_color->FlushRenderBindings(clear_encoder);
              Transfer::Rectangle clear_rect = *resolve_clear_rectangle;
              if (set_rect_viewport(clear_encoder, clear_rect)) {
                clear_encoder->drawPrimitives(MTL::PrimitiveTypeTriangle,
                                              NS::UInteger(0), NS::UInteger(3));
...
MTL::RenderPipelineState* MetalRenderTargetCache::GetOrCreateTransferPipelines(
  ...
  append_define(source, "XE_TRANSFER_FAST_DIVMOD",
                REXCVAR_GET(metal_transfer_fast_divmod) ? 1 : 0);
...
fragment TransferDepthOut transfer_ps(
  ...
  MTL::Function* ps = lib->newFunction(ps_name);
  if (!vs || !ps) {
    REXLOG_ERROR("GetOrCreateTransferPipelines: failed to get transfer shader entry");
    if (vs) vs->release();
    if (ps) ps->release();
    lib->release();
    return nullptr;
...
fragment TransferDepthOut transfer_clear_depth_ps(
  ...
  if (!transfer_library_) {
    REXLOG_ERROR("GetOrCreateTransferLibrary: failed to compile transfer library: {}",
           error && error->localizedDescription()
               ? error->localizedDescription()->utf8String()
               : "unknown error");
  }
...
MetalRenderTargetCache::GetOrCreateTransferClearPipeline(
  ...
  if (!pipeline) {
    REXLOG_ERROR("GetOrCreateTransferClearPipeline: failed to create pipeline: {}",
           error && error->localizedDescription()
               ? error->localizedDescription()->utf8String()
               : "unknown error");
    return nullptr;
...
MTL::DepthStencilState* MetalRenderTargetCache::GetTransferDepthStencilState(
  ...
  desc->setDepthCompareFunction(REXCVAR_GET(depth_transfer_not_equal_test)
