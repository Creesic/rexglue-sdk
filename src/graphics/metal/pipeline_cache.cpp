...
using BYTE = uint8_t;
#include <d3d12_5_1_bytecode/adaptive_quad_hs.h>
#include <d3d12_5_1_bytecode/adaptive_triangle_hs.h>
#include <d3d12_5_1_bytecode/continuous_quad_1cp_hs.h>
#include <d3d12_5_1_bytecode/continuous_quad_4cp_hs.h>
#include <d3d12_5_1_bytecode/continuous_triangle_1cp_hs.h>
#include <d3d12_5_1_bytecode/continuous_triangle_3cp_hs.h>
#include <d3d12_5_1_bytecode/discrete_quad_1cp_hs.h>
#include <d3d12_5_1_bytecode/discrete_quad_4cp_hs.h>
#include <d3d12_5_1_bytecode/discrete_triangle_1cp_hs.h>
#include <d3d12_5_1_bytecode/discrete_triangle_3cp_hs.h>
#include <d3d12_5_1_bytecode/tessellation_adaptive_vs.h>
#include <d3d12_5_1_bytecode/tessellation_indexed_vs.h>
...
void LogMetalErrorDetails(const char* label, NS::Error* error) {
  ...
  REXLOG_ERROR("{}: domain={} code={} desc='{}' failure='{}' recovery='{}'", label,
...
REXPACKEDSTRUCT(PipelineDiskCacheHeader, {
  uint32_t magic;
  uint32_t version;
  uint32_t reserved[2];
});

REXPACKEDSTRUCT(PipelineDiskCacheEntryHeader, {
  uint32_t entry_size;
  uint32_t reserved;
});

REXPACKEDSTRUCT(PipelineDiskCacheEntryBase, {
...
bool MetalPipelineCache::InitializeShaderTranslation(
  ...
  std::fprintf(stderr,
               "[probe] MetalPipelineCache::InitializeShaderTranslation enter gamma=%d msaa2x=%d scale=%ux%u\n",
               int(gamma_render_target_as_unorm8), int(msaa_2x_supported),
  ...
  if (!dxbc_to_dxil_converter_->Initialize()) {
    std::fprintf(stderr,
                 "[probe] MetalPipelineCache::InitializeShaderTranslation dxbc_to_dxil init failed\n");
    std::fflush(stderr);
    return false;
  }
  std::fprintf(stderr,
               "[probe] MetalPipelineCache::InitializeShaderTranslation dxbc_to_dxil ok\n");
  std::fflush(stderr);

  metal_shader_converter_ = std::make_unique<MetalShaderConverter>();
  if (!metal_shader_converter_->Initialize()) {
    std::fprintf(stderr,
                 "[probe] MetalPipelineCache::InitializeShaderTranslation metal_shader_converter init failed\n");
    std::fflush(stderr);
    return false;
  }
  std::fprintf(stderr,
               "[probe] MetalPipelineCache::InitializeShaderTranslation metal_shader_converter ok\n");
  std::fflush(stderr);
  ...
  int32_t thread_count_config = REXCVAR_GET(metal_pipeline_creation_threads);
  ...
  if (thread_count > 0 && REXCVAR_GET(async_shader_compilation)) {
    creation_threads_.reserve(thread_count);
    for (uint32_t i = 0; i < thread_count; ++i) {
      creation_threads_.emplace_back(&MetalPipelineCache::CreationThread, this,
                                     i);
    }
    REXLOG_INFO("MetalPipelineCache: spawned {} async pipeline creation threads",
           thread_count);
  }

  std::fprintf(stderr,
               "[probe] MetalPipelineCache::InitializeShaderTranslation success threads=%u\n",
               thread_count);
  std::fflush(stderr);
...
bool MetalPipelineCache::InitializeShaderStorageInternal(
  ...
  if (!device_) {
    REXLOG_WARN("Metal shader storage init skipped (no device)");
    return false;
  }
  ...
  if (ec) {
    REXLOG_WARN("Metal shader storage: Failed to create {}: {}",
  ...
  if (REXCVAR_GET(metal_pipeline_disk_cache)) {
    LoadPipelineDiskCache(pipeline_disk_cache_path_,
                          &pipeline_disk_cache_entries_);
  }

  if (REXCVAR_GET(metal_pipeline_binary_archive)) {
    ...
    if (pipeline_binary_archive_) {
      if (pipeline_binary_archive_dirty_) {
        ...
        if (!pipeline_binary_archive_->serializeToURL(url, &error)) {
          if (error) {
            REXLOG_WARN("Metal binary archive serialize failed: {}",
                   error->localizedDescription()->utf8String());
          }
        }
...
bool MetalPipelineCache::LoadPipelineDiskCache(
  ...
  pipeline_disk_cache_file_ = rex::filesystem::OpenFile(path, "a+b");
  if (!pipeline_disk_cache_file_) {
    REXLOG_WARN("Metal pipeline disk cache: Failed to open {}", path.string());
    return false;
  }
  ...
  if (std::fread(&header, sizeof(header), 1, pipeline_disk_cache_file_) != 1 ||
    ...
    header.reserved[0] = 0;
    header.reserved[1] = 0;
    rex::filesystem::Seek(pipeline_disk_cache_file_, 0, SEEK_SET);
    std::fwrite(&header, sizeof(header), 1, pipeline_disk_cache_file_);
    std::fflush(pipeline_disk_cache_file_);
    rex::filesystem::Seek(pipeline_disk_cache_file_, 0, SEEK_END);
    return true;
  }
...
bool MetalPipelineCache::InitializePipelineBinaryArchive(
  ...
  if (ec) {
    REXLOG_WARN("Metal binary archive existence check failed for {}: {}",
  ...
  if (!pipeline_binary_archive_) {
    if (error) {
      REXLOG_WARN("Metal binary archive init failed: {}",
             error->localizedDescription()->utf8String());
    }
    return false;
  }
...
Shader* MetalPipelineCache::LoadShader(xenos::ShaderType shader_type,
  ...
  REXLOG_DEBUG("Loaded {} shader at {:08X} ({} dwords, hash {:016X})",
...
bool MetalPipelineCache::EnsureDepthOnlyPixelShader() {
  ...
  if (!shader_translator_ || !dxbc_to_dxil_converter_ ||
    ...
    REXLOG_ERROR("Depth-only PS: shader translation not initialized");
    return false;
  }
  ...
  if (dxbc_data.empty()) {
    REXLOG_ERROR("Depth-only PS: failed to create DXBC");
    return false;
  }
  ...
  if (!dxbc_to_dxil_converter_->Convert(dxbc_data, dxil_data, &dxil_error)) {
    REXLOG_ERROR("Depth-only PS: DXBC to DXIL conversion failed: {}", dxil_error);
    return false;
  }
  ...
  if (!metal_shader_converter_->ConvertWithStage(MetalShaderStage::kFragment,
                                                 dxil_data, result)) {
    REXLOG_ERROR("Depth-only PS: DXIL to Metal conversion failed: {}",
  ...
  if (!depth_only_pixel_library_) {
    REXLOG_ERROR("Depth-only PS: Failed to create Metal library: {}",
           error ? error->localizedDescription()->utf8String() : "unknown");
    return false;
  }
  depth_only_pixel_function_name_ = result.function_name;
  if (depth_only_pixel_function_name_.empty()) {
    REXLOG_ERROR("Depth-only PS: missing function name");
    return false;
  }
  return true;
}
...
MetalPipelineCache::GetOrCreatePipelineState(
  ...
  if (!vertex_translation || !vertex_translation->metal_function()) {
    REXLOG_ERROR("No valid vertex shader function");
    return nullptr;
  }
  ...
  bool record_disk_entry =
      REXCVAR_GET(metal_pipeline_disk_cache) && pipeline_disk_cache_file_;
...
MTL::RenderPipelineState* MetalPipelineCache::CreatePipelineFromHandle(
  ...
  if (!pipeline) {
    if (error) {
      ...
      REXLOG_ERROR("Failed to create pipeline state (unknown error)");
    }
    return nullptr;
  }
...
MetalPipelineCache::GetOrCreateGeometryPipelineState(
  ...
  if (!vertex_translation) {
    REXLOG_ERROR("No valid vertex shader translation for geometry pipeline");
    return nullptr;
  }
  ...
  if (use_fallback_pixel_shader) {
    if (!EnsureDepthOnlyPixelShader()) {
      REXLOG_ERROR("Geometry pipeline: failed to create depth-only PS");
      return nullptr;
    }
    pixel_library = depth_only_pixel_library_;
    pixel_function = depth_only_pixel_function_name_.c_str();
  } else if (!pixel_library) {
    REXLOG_ERROR("No valid pixel shader translation for geometry pipeline");
    return nullptr;
  }
  ...
  auto get_vertex_stage = [&]() -> GeometryVertexStageState* {
    ...
    if (dxil_data.empty()) {
      ...
      if (!dxbc_to_dxil_converter_->Convert(
              ...
              &dxil_error)) {
        REXLOG_ERROR("Geometry VS: DXBC to DXIL conversion failed: {}", dxil_error);
        return nullptr;
      }
    }
    ...
    if (!vertex_reflection.vertex_inputs.empty()) {
      ...
      for (const auto& input : vertex_reflection.vertex_inputs) {
        ...
        if (input.attribute_index >= attribute_map.size()) {
          REXLOG_WARN("Geometry VS: vertex input {} out of range (max {})",
        ...
        if (mapped.format == IRFormatUnknown) {
          REXLOG_WARN("Geometry VS: unknown IRFormat for vertex input {}",
    ...
    if (!vertex_library) {
      REXLOG_ERROR("Geometry VS: Failed to create Metal library: {}",
    ...
    if (!stage_in_library) {
      REXLOG_ERROR("Geometry VS: Failed to create stage-in library: {}",
             stage_in_error
  ...
  auto get_geometry_stage = [&]() -> GeometryShaderStageState* {
    ...
    if (!dxbc_to_dxil_converter_->Convert(dxbc_bytes, dxil_data, &dxil_error)) {
      REXLOG_ERROR("Geometry GS: DXBC to DXIL conversion failed: {}", dxil_error);
      return nullptr;
    }
    ...
    if (!geometry_library) {
      REXLOG_ERROR("Geometry GS: Failed to create Metal library: {}",
             error ? error->localizedDescription()->utf8String()
                   : "unknown error");
      return nullptr;
    }
    ...
    if (state.max_input_primitives_per_mesh_threadgroup == 0) {
      REXLOG_ERROR("Geometry GS: reflection returned zero max input primitives");
    }
...
MetalPipelineCache::GetOrCreateTessellationPipelineState(
  ...
  if (!domain_translation) {
    REXLOG_ERROR("No valid domain shader translation for tessellation pipeline");
    return nullptr;
  }
  bool use_fallback_pixel_shader = (pixel_translation == nullptr);
  MTL::Library* pixel_library =
  ...
  if (use_fallback_pixel_shader) {
    if (!EnsureDepthOnlyPixelShader()) {
      REXLOG_ERROR("Tessellation pipeline: failed to create depth-only PS");
      return nullptr;
    }
    pixel_library = depth_only_pixel_library_;
    pixel_function = depth_only_pixel_function_name_.c_str();
  } else if (!pixel_library) {
    REXLOG_ERROR("No valid pixel shader translation for tessellation pipeline");
    return nullptr;
  }
  ...
  auto get_vertex_stage = [&]() -> TessellationVertexStageState* {
    ...
    std::string dxil_error;
    if (!dxbc_to_dxil_converter_->Convert(dxbc_bytes, dxil_data, &dxil_error)) {
      REXLOG_ERROR("Tessellation VS: DXBC to DXIL conversion failed: {}", dxil_error);
      return nullptr;
    }
    ...
    if (!metal_shader_converter_->ConvertWithStageEx(
            ...
            &vertex_reflection, nullptr, nullptr, true,
            static_cast<int>(input_topology))) {
      REXLOG_ERROR("Tessellation VS: DXIL to Metal conversion failed: {}",
             vertex_result.error_message);
      return nullptr;
    }
    ...
    if (!vertex_reflection.vertex_inputs.empty()) {
      ...
      for (const auto& input : vertex_reflection.vertex_inputs) {
        ...
        if (input.attribute_index >= attribute_map.size()) {
          REXLOG_WARN("Tessellation VS: vertex input {} out of range (max {})",
                 input.attribute_index, attribute_map.size());
          continue;
        }
        const InputAttribute& mapped = attribute_map[input.attribute_index];
        if (mapped.format == IRFormatUnknown) {
          REXLOG_WARN("Tessellation VS: unknown IRFormat for vertex input {}",
                 input.attribute_index);
          continue;
        }
        std::string semantic_base = input.name;
    ...
    if (stage_in_metallib.empty()) {
      REXLOG_ERROR("Tessellation VS: Failed to synthesize stage-in function");
      return nullptr;
    }
    ...
    dispatch_release(vertex_data);
    if (!vertex_library) {
      REXLOG_ERROR("Tessellation VS: Failed to create Metal library: {}",
             error ? error->localizedDescription()->utf8String()
                   : "unknown error");
      return nullptr;
    }
    ...
    dispatch_release(stage_in_data);
    if (!stage_in_library) {
      REXLOG_ERROR("Tessellation VS: Failed to create stage-in library: {}",
             stage_in_error
                 ? stage_in_error->localizedDescription()->utf8String()
                 : "unknown error");
      vertex_library->release();
    ...
    state.vertex_output_size_in_bytes =
        vertex_reflection.vertex_output_size_in_bytes;
    if (state.vertex_output_size_in_bytes == 0) {
      REXLOG_ERROR("Tessellation VS: reflection returned zero output size");
    }
  ...
  auto get_hull_stage = [&]() -> TessellationHullStageState* {
    ...
    switch (tessellation_mode) {
      ...
      case xenos::TessellationMode::kAdaptive:
        ...
        break;
      default:
        REXLOG_ERROR("Tessellation HS: unsupported tessellation mode {}",
               uint32_t(tessellation_mode));
        return nullptr;
    }
    ...
    std::string dxil_error;
    if (!dxbc_to_dxil_converter_->Convert(dxbc_bytes, dxil_data, &dxil_error)) {
      REXLOG_ERROR("Tessellation HS: DXBC to DXIL conversion failed: {}", dxil_error);
      return nullptr;
    }
    ...
    if (!metal_shader_converter_->ConvertWithStageEx(
      ...
      REXLOG_ERROR("Tessellation HS: DXIL to Metal conversion failed: {}",
    ...
    if (!hull_library) {
      REXLOG_ERROR("Tessellation HS: Failed to create Metal library: {}",
             error ? error->localizedDescription()->utf8String()
                   : "unknown error");
      return nullptr;
    }
    ...
    if (!state.reflection.has_hull_info) {
      REXLOG_ERROR("Tessellation HS: reflection missing hull info");
    }
  ...
  auto get_domain_stage = [&]() -> TessellationDomainStageState* {
    ...
    if (dxil_data.empty()) {
      ...
      if (!dxbc_to_dxil_converter_->Convert(
              ...
              &dxil_error)) {
        REXLOG_ERROR("Tessellation DS: DXBC to DXIL conversion failed: {}",
               dxil_error);
        return nullptr;
      }
    }
    ...
    if (!metal_shader_converter_->ConvertWithStageEx(
            ...
            static_cast<int>(IRInputTopologyUndefined))) {
      REXLOG_ERROR("Tessellation DS: DXIL to Metal conversion failed: {}",
    ...
    if (!domain_library) {
      REXLOG_ERROR("Tessellation DS: Failed to create Metal library: {}",
             error ? error->localizedDescription()->utf8String()
                   : "unknown error");
      return nullptr;
    }
    ...
    if (!state.reflection.has_domain_info) {
      REXLOG_ERROR("Tessellation DS: reflection missing domain info");
    }
  ...
  switch (hull_stage->reflection.hs_tessellator_output_primitive) {
    ...
    case IRRuntimeTessellatorOutputTriangleCCW:
      ...
      break;
    default:
      REXLOG_ERROR("Tessellation pipeline: unsupported tessellator output {}",
  ...
  if (!IRRuntimeValidateTessellationPipeline(
    ...
    REXLOG_ERROR("Tessellation pipeline: validation failed for HS/DS pairing");
    return nullptr;
  }
