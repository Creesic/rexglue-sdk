  ...
  for (const StoredLevelHostLayout& stored_level : stored_levels) {
    ...
    for (uint32_t slice = 0; slice < array_size; ++slice) {
      ...
      constants.is_tiled_3d_endian_scale =
          uint32_t(uint32_t(key.tiled)) | (uint32_t(is_3d_tiling) << 1) |
...
bool MetalTextureCache::Initialize() {
  // profile;
  // autorelease pool("MetalTextureCache::Initialize");
...
MTL::Texture* MetalTextureCache::CreateNullTexture2D() {
  ...
  MTL::Device* device = command_processor_->GetMetalDevice();
  if (!device) {
    REXLOG_ERROR("Metal texture cache: Failed to get Metal device for null texture");
    return nullptr;
  }
  ...
  if (!binding) {
    if (fetch_constant < logged_missing_binding.size() &&
      ...
      REXLOG_WARN(
          ...
          uint32_t(fetch.swizzle), fetch.dword_0, fetch.dword_1, fetch.dword_2,
          fetch.dword_3, fetch.dword_4, fetch.dword_5);
      if (decoded_key.is_valid) {
        const char* format_name = FormatInfo::Get(decoded_key.format)->name;
        REXLOG_WARN(
            ...
            decoded_key.GetDepthOrArraySize(), uint32_t(decoded_key.pitch),
  ...
  if (!texture) {
    ...
    if (fetch_constant < logged_missing_texture.size() &&
      ...
      REXLOG_WARN(
          ...
          FormatInfo::Get(binding->key.format)->name, binding->swizzled_signs,
...
MTL::Texture* MetalTextureCache::RequestSwapTexture(
  ...
  auto log_swap_failure_once =
      [&](SwapFailure reason, const TextureKey& log_key, const char* detail) {
        ...
        REXLOG_WARN(
            ...
            log_key.GetHeight(), uint32_t(log_key.pitch), log_key.mip_max_level + 1,
  ...
  if (!key.is_valid || key.base_page == 0 ||
    ...
    if (!logged_invalid) {
      REXLOG_WARN("MetalSwap: fetch0 invalid (valid={}, base_page=0x{:X}, dim={})",
             key.is_valid ? 1 : 0, uint32_t(key.base_page),
  ...
  uint32_t host_swizzle =
      GuestToHostSwizzle(uint32_t(fetch.swizzle), GetHostFormatSwizzle(key));
