...
void BuilderContext::emit_conditional_branch(bool not_, std::string_view cond) {
  ...
  // Backward branch to the entry of the function currently being emitted is a local
  // loop, even if graph lookup would classify the target as another Function entry
  // (e.g. gap-fill chunks overlapping the same address range).
  if (target == fn.base()) {
