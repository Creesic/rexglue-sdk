...
bool build_vpkd3d128(BuilderContext& ctx) {
  ...
  switch (ctx.insn.operands[2]) {
    ...
    case 5:  // float16_4
    {
      ...
      // In-place packing (vpkd3d128 vD, vD, 5, ...) is common: dst == src, and
      // the lane-by-lane u16 stores below clobber u32 source lanes that later
      // iterations still read, so half of every FLOAT16_4 output packed as
      // 0x0000 (silent post-splash audio: FMOD channel gains converted in place
      // all came out as 0). Snapshot the four source lanes first and read only
      // from the snapshot. The enclosing scope keeps _vpks* from colliding when
      // a single function contains multiple case-5 packs.
      ctx.println("\t{{");
      for (size_t i = 0; i < 4; i++) {
