...
Result<void> ProjectRecompiler::Run(const ProjectRecompilerOptions& opts) {
  ...
  for (size_t i = 0; i < contexts.size(); ++i) {
    ...
    // Bootstrap-discovered stubs feed the config includes below, so a new
    // discovery must invalidate the stamp even before any include changes.
    if (!entry.module->configIncludes.empty() && fs::exists(discovered_path))
      inputs.push_back(discovered_path);
