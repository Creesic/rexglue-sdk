...
class XAudio2AudioDriver::VoiceCallback final : public IXAudio2VoiceCallback {
 public:
  ...
  void OnVoiceError([[maybe_unused]] void* context,
                    [[maybe_unused]] HRESULT result) noexcept override {}
...
XAudio2AudioDriver::XAudio2AudioDriver(rex::memory::Memory* memory,
                                       rex::thread::Semaphore* semaphore, uint32_t frequency,
...
bool XAudio2AudioDriver::Initialize() {
  ...
  xaudio2_create_ =
      reinterpret_cast<XAudio2CreateFn>(GetProcAddress(xaudio2_module_, "XAudio2Create"));
  if (!xaudio2_create_) {
...
bool XAudio2AudioDriver::InitializeObjects() {
  ...
  if (FAILED(hr)) {
    REXAPU_ERROR("IXAudio2::CreateMasteringVoice failed with 0x{:08X}", static_cast<uint32_t>(hr));
    ShutdownObjects();
    return false;
  }
