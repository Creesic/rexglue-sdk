...
__attribute__((format(printf, 1, 2))) void RexCapLog(const char* fmt, ...) {
  ...
  fprintf(stderr, "[rex-cap] %s.%03d pid=%d ", timebuf, int(tv.tv_usec / 1000), getpid());
  ...
  if (f) {
    fprintf(f, "[rex-cap] %s.%03d pid=%d ", timebuf, int(tv.tv_usec / 1000), getpid());
...
void LogNSError(NSError* error) {
  ...
  RexCapLog("NSError localizedFailureReason=%s", NSStr(error.localizedFailureReason));
  ...
  for (id key in info) {
    id value = info[key];
    RexCapLog("NSError userInfo[%s]=%s", NSStr([key description]), NSStr([value description]));
  }
}
...
extern "C" void RexMetalCaptureDiagFileStatus(const char* output_path) {
  ...
  if (stat(path, &st) == 0) {
    ...
    const char* type = is_dir ? "directory/package" : (is_reg ? "regular-file" : "other");
    const uint64_t recursive_bytes = is_dir ? ComputeRecursiveBytes(std::filesystem::path(path)) : 0;
    RexCapLog(
        "file status: exists path=%s type=%s stat_size=%lld recursive_size=%llu mode=%o mtime=%lld",
        path, type, (long long)st.st_size, static_cast<unsigned long long>(recursive_bytes),
        (unsigned)st.st_mode, (long long)st.st_mtime);
  } else {
    RexCapLog("file status: missing path=%s errno=%d strerror=%s", path, errno, strerror(errno));
  }
}
...
extern "C" bool RexMetalCaptureDiagStart(void* capture_object_ptr, const char* output_path) {
  @autoreleasepool {
    ...
    RexCapLog("START requested captureObjectPtr=%p outputPath=%s", capture_object_ptr, path);
    RexCapLog("env MTL_CAPTURE_ENABLED=%s", EnvOrUnset("MTL_CAPTURE_ENABLED"));
    RexCapLog("env REX_METAL_CAPTURE_TO_FILE=%s", EnvOrUnset("REX_METAL_CAPTURE_TO_FILE"));
    ...
    id metal_capture_enabled = [bundle objectForInfoDictionaryKey:@"MetalCaptureEnabled"];
    RexCapLog("bundle path=%s", NSStr(bundle.bundlePath));
    RexCapLog("Info.plist MetalCaptureEnabled=%s", NSStr([metal_capture_enabled description]));
    ...
    RexCapLog("captureObject class=%s description=%s", object_getClassName(capture_object),
              NSStr([capture_object description]));

    BOOL is_device = [capture_object conformsToProtocol:@protocol(MTLDevice)];
    BOOL is_queue = [capture_object conformsToProtocol:@protocol(MTLCommandQueue)];
    BOOL is_scope = [capture_object conformsToProtocol:@protocol(MTLCaptureScope)];
    ...
    if (is_device) {
      ...
      RexCapLog("captureObject MTLDevice name=%s registryID=%llu", NSStr(device.name),
                (unsigned long long)device.registryID);
    }
    if (is_queue) {
      id<MTLCommandQueue> queue = (id<MTLCommandQueue>)capture_object;
      RexCapLog("captureObject MTLCommandQueue deviceName=%s", NSStr(queue.device.name));
    }

    MTLCaptureManager* manager = [MTLCaptureManager sharedCaptureManager];
    BOOL supports_gpu_trace = [manager supportsDestination:MTLCaptureDestinationGPUTraceDocument];
    BOOL supports_tools = [manager supportsDestination:MTLCaptureDestinationDeveloperTools];
    RexCapLog(
        "manager=%p isCapturing(before)=%d supportsGPUTraceDocument=%d supportsDeveloperTools=%d",
        manager, manager.isCapturing ? 1 : 0, supports_gpu_trace ? 1 : 0, supports_tools ? 1 : 0);
    ...
    BOOL made_dir = [[NSFileManager defaultManager] createDirectoryAtPath:dir
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:ns_path]) {
      ...
      BOOL removed =
          [[NSFileManager defaultManager] removeItemAtPath:ns_path error:&remove_error];
    ...
    desc.outputURL = url;
    RexCapLog("descriptor captureObject=%p destination=%ld outputURL=%s", desc.captureObject,
              (long)desc.destination, NSStr(desc.outputURL.path));
...
extern "C" void RexMetalCaptureDiagStop(const char* output_path) {
  @autoreleasepool {
    ...
    RexCapLog("STOP complete isCapturing(after)=%d", manager.isCapturing ? 1 : 0);
