...
// Generated shaders
#include "rex/ui/shaders/bytecode/metal/immediate_ps.h"
#include "rex/ui/shaders/bytecode/metal/immediate_vs.h"
...
static MTL::Function* CreateFunction(MTL::Device* device, const void* data, size_t size,
  ...
  dispatch_release(d);
  if (!lib) {
    REXLOG_ERROR("MetalImmediateDrawer: Failed to create library for {}: {}", name,
  ...
  lib->release();
  if (!fn) {
    REXLOG_ERROR("MetalImmediateDrawer: Failed to load function entry_xe for {}", name);
  }
  return fn;
}
...
bool MetalImmediateDrawer::Initialize() {
  ...
  if (!white_texture_) {
    REXLOG_ERROR("MetalImmediateDrawer: Failed to create white texture");
    return false;
  }
  ...
  if (!pipeline_textured_) {
    REXLOG_ERROR("MetalImmediateDrawer: Failed to create pipeline: {}",
           error ? error->localizedDescription()->utf8String() : "unknown");
    return false;
  }
...
void MetalImmediateDrawer::Shutdown() {
  ...
  REXLOG_INFO("Metal immediate drawer shut down");
}
...
std::unique_ptr<ImmediateTexture> MetalImmediateDrawer::CreateTexture(uint32_t width,
  ...
  if (!metal_texture) {
    REXLOG_ERROR("Metal CreateTexture failed to create {}x{} texture", width, height);
    return nullptr;
  }
  ...
  if (!sampler_state) {
    REXLOG_ERROR("Metal CreateTexture failed to create sampler state");
...
void MetalImmediateDrawer::BeginDrawBatch(const ImmediateDrawBatch& batch) {
  ...
  if (!vertex_buffer) {
    REXLOG_ERROR("Metal immediate drawer failed to create vertex buffer for {} vertices",
           batch.vertex_count);
    return;
  }
