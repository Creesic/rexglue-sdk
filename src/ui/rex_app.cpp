  ...
  if (imgui_drawer_) {
    ...
    if (input_sys) {
      input_sys->SetActiveCallback([this]() {
        if (!debug_overlay_ && !console_overlay_ && !audio_voices_overlay_ &&
            !settings_overlay_ && !achievements_overlay_)
          return true;
    ...
    audio_voices_overlay_ = std::make_unique<ui::AudioVoicesDialog>(
        imgui_drawer_.get(), [this]() -> ui::AudioVoicesSnapshot {
          ...
          for (size_t i = 0; i < audio::AudioSystem::kMaximumClientCount; ++i) {
            ...
            out.voices.push_back(ui::AudioVoiceInfo{static_cast<uint32_t>(i), client.driver_handle,
                                                    client.callback, client.callback_arg,
                                                    client.submitted_frames});
          }
          ...
          if (auto* xma_decoder = audio_system->xma_decoder()) {
            ...
            out.vp.decode_iterations_per_second = xma_snapshot.vp.decode_iterations_per_second;
            ...
            for (size_t i = 0; i < audio::XmaDecoder::kContextCount; ++i) {
              ...
              out.xma_contexts.push_back(ui::AudioXmaContextInfo{
                  ...
                  in.input1_valid, in.output_valid, in.stop_when_done, in.interrupt_when_done,
                  in.consume_only, in.stereo, in.muted, in.volume, in.peak_level, in.rms_level,
                  in.audible_peak_level, in.audible_rms_level, in.current_buffer,
                  in.subframe_decode_count, in.output_buffer_block_count,
                  in.output_buffer_write_offset, in.output_buffer_read_offset, in.sample_rate_id,
                  in.loop_count, in.output_buffer_padding, in.loop_subframe_end,
                  in.loop_subframe_skip, in.packet_metadata,
                  in.input_buffer_0_packet_count, in.input_buffer_1_packet_count, in.guest_ptr,
                  in.input_buffer_read_offset, in.input_buffer_0_ptr, in.input_buffer_1_ptr,
                  in.output_buffer_ptr});
            }
          }
          return out;
        },
        ...
        [this](const ui::AudioVoicesLayoutState& state) {
          ...
          SaveAudioVoicesLayoutState(user_data_root_, runtime_->kernel_state()->title_id(), state);
        });
  });
