  ...
  switch (event.type) {
    ...
    case SDL_EVENT_WINDOW_CLOSE_REQUESTED:
      ...
      if (SendCloseRequestToListeners(destruction_receiver)) {
        if (!destruction_receiver.IsWindowDestroyed()) {
          // Keep the FM2 runtime log explicit about user-requested shutdowns
          // so a normal close isn't mistaken for a crash in post-run analysis.
          REXLOG_INFO("REXUI_WINDOW_CLOSED_BY_USER msg=SDL_EVENT_WINDOW_CLOSE_REQUESTED");
          if (FILE* close_log = std::fopen("C:\\temp\\fm2-clean.log", "a")) {
            std::fputs(
                "REXUI_WINDOW_CLOSED_BY_USER msg=SDL_EVENT_WINDOW_CLOSE_REQUESTED "
                "(user closed the window; NOT a crash)\n",
                close_log);
