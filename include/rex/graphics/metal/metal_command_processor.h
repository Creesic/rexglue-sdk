...
#include "rex/graphics/metal/metal_geometry_shader.h"
#include "rex/graphics/metal/metal_pipeline_cache.h"
#include "rex/graphics/metal/metal_primitive_processor.h"
#include "rex/graphics/metal/metal_render_target_cache.h"
#include "rex/graphics/metal/metal_shader.h"
...
#include "rex/graphics/metal/metal_upload_buffer_pool.h"
#include "rex/graphics/metal/metal_zpd_visibility_pool.h"
// clang-format off
...
class MetalCommandProcessor final : public CommandProcessor {
 public:
  ...
  static constexpr ReportHandle kInvalidReportHandle = UINT64_MAX;
  ...
  static constexpr uint32_t kZPDQueryPoolCapacity = 1024;
 ...
 protected:
  ...
  void PollCompletedSubmission();
  void EnsureZPDQueryResources();
  void ShutdownZPDQueryResources();
  bool IsZPDQueryPoolReady() const;
  bool CanOpenZPDQuery() const;
  ...
  bool DiscardZPDQuery();
  void PumpQueryResolves();
  bool AwaitQueryResolve(ReportHandle report_handle,
                         uint64_t wait_for_submission);
  ZPDMode GetZPDMode() const { return ZPDMode::kFake; }
  void OnZPDQueryResolved(ReportHandle report_handle, uint64_t sample_count) {
    (void)report_handle;
    (void)sample_count;
  }
  ...
  void WriteFastRegisterRangeFromRing(memory::RingBuffer* ring, uint32_t base,
                                      uint32_t num_registers);
 ...
 private:
  ...
  struct LogicalZPDReport {
    uint32_t pending_segments = 0;
    bool ended = true;
  };
  std::unordered_map<ReportHandle, LogicalZPDReport> logical_zpd_reports_;
