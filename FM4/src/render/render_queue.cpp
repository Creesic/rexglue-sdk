...
struct Job {
  ...
  std::array<uint8_t, 12 * 16> liveVsConstants{};
  uint32_t liveVsConstantBytes = 0;
};
...
void ExecuteJob(Job& job) {
  if (job.replayBatch != nullptr) {
    DispatchRecordedRenderCommands(job.replayCommands.data(), job.replayCommands.size(),
                                   job.liveVsConstants.data(), job.liveVsConstantBytes);
  } else {
...
void ThreadMain() {
  ...
  REXGPU_INFO("FM4 render queue: render thread started");
  while (true) {
    Job job;
    {
  ...
  REXGPU_INFO("FM4 render queue: render thread stopped");
}
...
bool RenderQueue::ReplayRecording(uint32_t cloneAddress, const uint8_t* liveVsConstants,
  ...
  if (liveVsConstants != nullptr) {
    job.liveVsConstantBytes =
        std::min<uint32_t>(liveVsConstantBytes, uint32_t(job.liveVsConstants.size()));
    std::memcpy(job.liveVsConstants.data(), liveVsConstants, job.liveVsConstantBytes);
  }
  if (!g_running.load(std::memory_order_acquire) || IsOnRenderThread()) {
    ExecuteJob(job);
  } else {
