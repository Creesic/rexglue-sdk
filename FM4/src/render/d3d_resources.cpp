...
void ProcCreateSurfaceHost(GuestSurface* surface, uint32_t width, uint32_t height, uint32_t format,
  ...
  if (width == kFm4FrameWidth && height == kFm4TileHeight) {
    surface->tileGrownFromHeight = height;
    hostHeight = kFm4FrameHeight;
...
// objects carry no kFm4ResourceMagic tag, so D3DDevice_SetTexture can't bind
