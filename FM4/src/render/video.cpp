...
// submitting it.
constexpr uint32_t kNumFrames = fm4::render::kNumFrames;
...
uint32_t g_descriptorCapacity = fm4::render::kNullTextureDescriptorCount;
std::vector<uint32_t> g_freedDescriptors;
std::array<std::unique_ptr<RenderTexture>,
           fm4::render::kNullTextureDescriptorCount>
    g_nullTextures;
...
fm4::render::GuestBaseTexture* g_presentSource = nullptr;
...
void ExecuteCommandListImpl() {
  std::lock_guard lock(fm4::render::RecordingMutex());
  g_presentSubmitOk = false;
  if (fm4::render::IsDeviceLost()) return;
  ...
  // override (live scene RT) recorded when the copy was skipped.
  {
    ...
    ++presentSourceChecks;
    fm4::render::GuestBaseTexture* rt = fm4::render::ConsumeStretchRectPresentOverride();
    const char* kind = "stretch-src";
    if (rt == nullptr || rt->texture == nullptr) {
      rt = fm4::render::ConsumeFrontbufferPresentSource();
      kind = "aperture";
    }
    if (rt == nullptr || rt->texture == nullptr) {
      rt = fm4::render::GetCurrentColorRenderTarget();
      kind = "sticky-rt";
    }
    ...
    fm4::render::SetPresentSource(rt);
  }
  ...
  RenderPipeline* blitPipeline = fm4::render::GetBlitPipeline(kBackbufferFormat);
  ...
  if (blit) {
    if (g_presentSource->descriptorIndex == 0) {
      g_presentSource->descriptorIndex = fm4::render::AllocTextureDescriptor();
    }
...
// Vblank delivery lives in FM4/src/gpu/native_video.cpp: it maps the GPU MMIO
// window D3D::InterruptCallback reads and drives both the 60 Hz vblank
// (source 0) and one swap-completion interrupt (source 1) per presented frame.
// FM2P called an SDK helper here that this SDK does not have; do not add a
// second worker.
...
bool Video::Init(void* nativeWindowHandle, uint32_t width, uint32_t height) {
...
#if defined(_WIN32)
  // Opt-in GPU diagnostics: set FM4_GPU_DEBUG=1 to enable the D3D12 debug
  ...
  {
    const char* dbg = std::getenv("FM4_GPU_DEBUG");
    if (dbg != nullptr && dbg[0] != '\0' && dbg[0] != '0') {
      ID3D12Debug* debugController = nullptr;
      if (SUCCEEDED(D3D12GetDebugInterface(IID_PPV_ARGS(&debugController)))) {
        debugController->EnableDebugLayer();
        debugController->Release();
        REXGPU_INFO("FM4_GPU_DEBUG: D3D12 debug layer enabled");
      } else {
        REXGPU_WARN("FM4_GPU_DEBUG: D3D12 debug layer unavailable");
      }
      ...
      if (SUCCEEDED(D3D12GetDebugInterface(IID_PPV_ARGS(&dredSettings)))) {
        ...
        REXGPU_INFO("FM4_GPU_DEBUG: DRED breadcrumbs + page-fault tracking enabled");
      } else {
        REXGPU_WARN("FM4_GPU_DEBUG: DRED settings unavailable");
      }
    }
  }
#endif
...
#if defined(_WIN32)
  // With FM4_GPU_DEBUG on, keep the InfoQueue useful: FM2 clears RTs/DS every
  ...
  if (g_d3d12Backend && std::getenv("FM4_GPU_DEBUG") != nullptr) {
    ...
    if (d3dDevice->d3d != nullptr &&
      ...
      infoQueue->Release();
      REXGPU_INFO("FM4_GPU_DEBUG: InfoQueue clear-value warning spam muted");
    }
  }
#endif
  ...
  {
    using fm4::render::kNullTexture2DDescriptor;
    using fm4::render::kNullTexture3DDescriptor;
    using fm4::render::kNullTextureCubeDescriptor;
    using fm4::render::kNullTextureDescriptorCount;
    using fm4::render::kSamplerDescriptorSize;
    using fm4::render::kTextureDescriptorSize;
  ...
  fm4::render::RenderQueue::Start();
  for (uint32_t i = 0; i < kNumFrames; ++i) {
    fm4::render::OnRecordingFrameReady(i);
  }
  return true;
}
  ...
  if (!g_deviceLost.exchange(true, std::memory_order_acq_rel)) {
    ...
    if (g_d3d12Backend && g_device != nullptr) {
      ...
      if (device->d3d != nullptr) {
        ...
        // DRED (populated only when FM4_GPU_DEBUG enabled it at Init): name
...
bool Video::Present(fm4::render::GuestBaseTexture* frontBuffer) {
  if (!g_initialized || fm4::render::IsDeviceLost()) {
    return false;
  }

#if defined(_WIN32)
  // Auto-capture: with the RenderDoc dll injected (renderdoccmd capture) and
  // FM4_RDOC_CAPTURE_AT=<frame>, trigger a one-frame capture at that Present.
  {
    ...
    static const uint64_t captureAt = [] {
      const char* v = std::getenv("FM4_RDOC_CAPTURE_AT");
  ...
  if (frontBuffer != nullptr && frontBuffer->texture != nullptr) {
    fm4::render::SetFrontbufferPresentSource(frontBuffer);
  }
  ...
  fm4::render::RenderCommand cmd{};
  cmd.type = fm4::render::RenderCommandType::ExecuteCommandList;
  fm4::render::RenderQueue::Run(cmd);
...
void Video::WaitForGPU() {
  ...
  std::lock_guard lock(fm4::render::RecordingMutex());
  fm4::render::RenderCommand cmd{};
  cmd.type = fm4::render::RenderCommandType::WaitForGpu;
  fm4::render::RenderQueue::Run(cmd);
}
...
void ProcExecuteCommandList() {
  ...
  fm4::render::NotifyRenderFrameBegin();
}
