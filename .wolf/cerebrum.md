...
- **Rebase onto `origin/development` (2026-08-16):** upstream moved the `REX_MM_LOAD_*` macros out of `init_h.inja` into `pch_h.inja` (init_h is now an 8-line include stub), re-pinned spirv-headers/spirv-tools/vulkan-headers (commit `fcd1e661`), dropped the `snappy`/`volk` submodules, added APPLE-only `moltenvk`/`vulkan-loader`, and made the core libs (`rexcore rexfilesystem rexinput rexui rexaudio`) OBJECT libraries that are absorbed by `rexruntime` and NOT in `REXGLUE_INSTALL_TARGETS` — so anything exported must not PUBLIC-link them. `plume` keeps clang-cl + `-march=x86-64-v3` on Windows where development uses clang++ + v2.
- Configure needs a VS developer environment (`vcvars64.bat`): outside it, sdl3's `CheckHIDAPI` dies on a missing `dumpbin`.
