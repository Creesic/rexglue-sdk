...
- **FM2→Unleashed convergence (2026-07-12):** Phase A–C done; StretchRect/MSAA/create/SetTexture on RT; intermediary DrawUP + deferred destroy; Release/AddRef hooked; POD `RenderCommand`+`Proc*` for state setters (fn `Run` remains for Present/draws/creates). Still: POD draws/Present, drop RecordingMutex, optional moodycamel + shader MSAA resolve.
