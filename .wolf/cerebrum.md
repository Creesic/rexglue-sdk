...
- **FM2→Unleashed convergence (2026-07-12):** POD commands cover setters (incl. viewport-enable/depth/stencil)/draws/creates/Present split/Unlock+upload/Translate create. No remaining `Enqueue(fn)` lambdas in `FM2/src/render/`. Still optional: async ECL+ready gate, drop RecordingMutex, delete unused `Run(fn)`/`Enqueue(fn)` APIs.
