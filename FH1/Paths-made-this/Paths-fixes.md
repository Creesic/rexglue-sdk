...
## 8. FH1 work-queue dispatch AV at `sub_82C0BC88` (loading worker)

**Symptom:** Access violation reading guest `0x70B20000` in `sub_82C0BC88` (~generated line 51573–51576): `lwz r10,0(r10)` / `lwzx` through the job ring pointer at `*(queue+0)`. Stack: `sub_82C0BDC8` → `sub_82C0C668` → `sub_823ED888` (world-load worker from `sub_8255AE10`).

**Cause:** Worker reaches dispatch while the 72-byte job ring at `*(queue+0)` is not committed yet (or was resized/freed on another thread). A `stub_return = 1` stub avoided the AV but hung the loading screen because no job ran.

**Fix (2026-05-29):** `FH1/src/fh1_workqueue_dispatch.cpp` — validate/commit full ring `[*(queue+0), *(queue+4))` (72-byte slots), not just the first 72 bytes; rate-limited retry logging.

**Loading-screen hang vs window close (2026-05-29):** Log showed `sub_82C0BC88` AV → `sub_823ED888` worker abort → `sub_8255AE10` skipped teardown. Calling `sub_82C00E50` on **failed** worker wakes the main thread, which then hits `KernelState::TerminateTitle` — window closes but host audio/GPU threads keep running (zombie process). **Do not** signal `sub_82C00E50` unless `g_track_loader_worker_ok`; only call after successful worker + teardown.

**BC88 AV at host 0x170020000 (2026-05-30):** Maps to guest `0x70020000` — the fiber/job context block that `sub_830EBE90` → `sub_830ED910` saves registers into via `*(r13+256)+356`. Ring validation passed but the context page was not committed. Fix: commit fiber/job context before dispatch; guard `sub_830EBE90` / `sub_830ED910`; restore full `PPCContext` on BC88 SEH.

**Fiber context commit size (2026-05-30):** `sub_830ED910` `stvlxl128`/`lvxl128` touches offsets up to **0xA10+0x30** on r10 (=ctx+0x30). Commit **`sizeof(X_FIBER_CONTEXT)` (0xA50)** page-rounded in `fh1_workqueue_dispatch.cpp`.

**IDA `sub_830ED910` / KeSetCurrentStackPointers (2026-05-30):** Fiber swap saves current ctx to `*(TLS+0x164)` where TLS=`*(r13+0x100)`, restores job ctx from `r3`, then `KeSetCurrentStackPointers(sp, TLS, stack_alloc, stack_base, stack_limit)`. ReX was writing stack fields to the **r4 TLS block as if it were KTHREAD** — fixed in `KeSetCurrentStackPointers_entry` to use `current_thread->guest_object<X_KTHREAD>()` and sync host `XThread` stack span for `0x70000000` fiber stacks.

**Static-recomp fiber reentry (2026-05-30):** After KeSet the guest blrs to the job resume address; generated C++ must not unwind the nested host call stack (rexglue-sdk#50 / Xenia Reenter). `include/rex/ppc/static_recomp_fiber.h` + `KeSetCurrentStackPointers` longjmp back to the `sub_830ED910` wrapper, which dispatches `ctx.lr` on fiber stacks and longjmps to the outer frame on swap-back. **Do not** wrap `sub_830ED910`, `sub_82C013E0`, or `sub_82C0BC88` in `__try/__except` — MSVC reports `0xC0000028` or SEH `code=0` when longjmp/reentry crosses SEH frames. Fiber stack heuristic: `0x70000000–0x78000000` (not all of `0x7xxxxxxx`). Commit misaligned fiber contexts via page-rounded `EnsureGuestRangeCommitted`.

**Upstreamable:** `IsGuestVirtualCommitted` yes; title guard is FH1-specific unless pattern repeats.

---

## 9. FH1 world-load worker AV at `sub_82480C28` (null track-loader object)

**Symptom:** Access violation reading host `0x1000000026` in `sub_82480C28` (~`fh1_recomp.6.cpp` line 60379): `lbz r11,38(r3)` with `r3≈0`. Stack: `sub_8255AE10` → `sub_823ED888` → `sub_82480C28`.

**Cause:** `sub_8255AE10` allocates a 712-byte track-loader object via `sub_8240AC00`, stores it at `lis(-31954), -4060(r31)`, then calls `sub_823ED888` even when alloc returns null. `sub_823ED888` keeps the bad pointer in `r31` and later passes it to `sub_82480C28`.

**Fix (2026-05-29):** `FH1/src/fh1_world_load_guards.cpp` — strong overrides:
- `sub_8255AE10`: return early when alloc fails (do not run worker).
- `sub_823ED888`: resolve object from `r3` or global slot; skip if missing.
- `sub_82480C28`: restore `r3`/`r31` from global slot before dispatch; SEH + no-op if still invalid.
- **Follow-up:** `sub_823ED888` keeps the track-loader pointer in TLS; wrappers on `sub_82C0C668`, `sub_8247D7A8`, `sub_82C0EA68`, `sub_825D0640`, `sub_82553BC0`, `sub_82570C98`, and post-call repair in `sub_82480C28` restore `r31` after callees clobber it (AV at `lbz 245(r31)` ~line 34842).

**Status:** crash guard landed; root cause (why `sub_8240AC00(712)` fails or nested +3112 stays null) still open.

**Follow-up (2026-05-29):** teardown path `sub_824866F0` → `sub_824821F0` → `sub_8257E3A0` AV at `lwz 8(r11)` when `*(owner+3112)==0`. Guards added on `sub_8257E3A0`/`sub_8257E3B8`, `sub_824866F0`, and `sub_8255AE10` skips teardown when the worker did not complete.

**Upstreamable:** title-specific unless alloc failures are common across titles.
...
| 8 | `xmemory.{h,cpp}` + `FH1/src/fh1_workqueue_dispatch.cpp` | AV at `sub_82C0BC88` on loading worker | workaround | partial |
